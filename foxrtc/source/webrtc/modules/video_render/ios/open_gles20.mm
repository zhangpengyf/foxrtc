...
void OpenGles20::UpdateTextures(const VideoFrame& frame) {
  ...
  GlTexSubImage2D(width, height, frame.video_frame_buffer()->StrideY(), frame.video_frame_buffer()->DataY());

  glActiveTexture(GL_TEXTURE1);
  glBindTexture(GL_TEXTURE_2D, texture_ids_[1]);
  GlTexSubImage2D(
      width / 2, height / 2, frame.video_frame_buffer()->StrideU(), frame.video_frame_buffer()->DataU());

  glActiveTexture(GL_TEXTURE2);
  glBindTexture(GL_TEXTURE_2D, texture_ids_[2]);
  GlTexSubImage2D(
      width / 2, height / 2, frame.video_frame_buffer()->StrideV(), frame.video_frame_buffer()->DataV());
}
