...
void VideoChannelNSOpenGL::OnFrame(const VideoFrame& frame) {
  _owner->LockAGLCntx();

  if(_width != frame.width() ||
     _height != frame.height()) {
      if(FrameSizeChange(frame.width(), frame.height(), 1) == -1) {
        _owner->UnlockAGLCntx();
        return;
      }
  }
...
int VideoChannelNSOpenGL::DeliverFrame(const VideoFrame& videoFrame) {
  ...
  // TODO(wu): See if we are using glTexSubImage2D correctly.
    VideoFrame frame;
    frame.CreateFrame(videoFrame.video_frame_buffer()->DataY(), videoFrame.video_frame_buffer()->DataV(), videoFrame.video_frame_buffer()->DataU(), videoFrame.width(), videoFrame.height(), videoFrame.video_frame_buffer()->StrideY(), videoFrame.video_frame_buffer()->StrideV(), videoFrame.video_frame_buffer()->StrideU(), videoFrame.rotation());
    int rgbRet = ConvertFromI420(frame, kBGRA, 0, _buffer);
...
{
  _screenUpdateThread = new rtc::PlatformThread(
...
VideoRenderNSOpenGL::~VideoRenderNSOpenGL()
{
    ...
    rtc::PlatformThread* tmpPtr = _screenUpdateThread;
