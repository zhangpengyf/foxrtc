...
_ptrWindow(window)
{
    _blitzView = [[CocoaRenderView alloc]init];
    CocoaRenderView* blitzView = (CocoaRenderView*)_blitzView;
    NSView* view = (NSView*)_ptrWindow;
    blitzView.frame = view.bounds;
    blitzView.bounds = view.bounds;
    [view addSubview:blitzView];
...
VideoRenderMacCocoaImpl::Init()
{
    ...
    _ptrCocoaRender = new VideoRenderNSOpenGL((CocoaRenderView*)_blitzView, _fullScreen, _id);
    if (!_ptrWindow)
    {
...
VideoRenderMacCocoaImpl::ChangeWindow(void* window)
{
    ...
    _ptrWindow = window;
    //由于yysdk 的view放到BlitzRenderViewMac就不出人，所以改为外面传原始的view，在这里创建CocoaRenderViewBlitz
    if (_blitzView) {
        CocoaRenderView* blitzView = (CocoaRenderView*)_blitzView;
        [blitzView removeFromSuperview];
        _blitzView = nil;
    ...
    blitzView.bounds = view.bounds;
    [view addSubview:blitzView];
    _ptrCocoaRender->ChangeWindow((CocoaRenderView*)_blitzView);
    return 0;
}
