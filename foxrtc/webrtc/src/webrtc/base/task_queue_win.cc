...
BOOL CALLBACK InitializeTls(void* param, void** context) {
...
DWORD GetQueuePtrTls() {
	...
	else if (InterlockedCompareExchange(&flag, 1, 0) == 0) {
		InitializeTls(nullptr, nullptr);
		InterlockedCompareExchange(&flag, 2, 1);
	}
	else {
		while (flag != 2) {
			Sleep(1);
