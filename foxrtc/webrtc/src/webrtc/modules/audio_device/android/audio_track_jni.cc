...
AudioTrackJni::JavaAudioTrack::JavaAudioTrack(
    ...
    : audio_track_(std::move(audio_track)),
      ...
      get_stream_max_volume_(native_reg->GetMethodId("getStreamMaxVolume", "()I")),
      get_stream_volume_(native_reg->GetMethodId("getStreamVolume", "()I")),
      speaker_status_(native_reg->GetMethodId("setPlayoutSpeaker", "(Z)I")) {}
...
int AudioTrackJni::JavaAudioTrack::SetLoudspeakerStatus(bool enable) {
  return audio_track_->CallIntMethod(speaker_status_,enable);
}
...
AudioTrackJni::AudioTrackJni(AudioManager* audio_manager)
  ...
  j_native_registration_ = j_environment_->RegisterNatives(
      "com/talk51/blitz/webrtc/WebRtcAudioTrack", native_methods,
...
int AudioTrackJni::SetLoudspeakerStatus(bool enable) {
  ALOGD("SetLoudspeakerStatus%s", GetThreadInfo().c_str());
  RTC_DCHECK(thread_checker_.CalledOnValidThread());
  RTC_DCHECK(initialized_);
  if (j_audio_track_->SetLoudspeakerStatus(enable) != 0) {
    ALOGE("SetLoudspeakerStatus failed!");
    return -1;
  }
  loudSpeakerOn_ = enable;
  return 0;
}

int AudioTrackJni::GetLoudspeakerStatus(bool& enabled) const {  // NOLINT
  enabled = loudSpeakerOn_;
  return 0;
}
