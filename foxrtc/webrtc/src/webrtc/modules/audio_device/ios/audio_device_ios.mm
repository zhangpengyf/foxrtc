...
static bool g_audioDeviceIsValid = false;
namespace webrtc {
...
void AudioDeviceIOS::OnInterruptionBegin() {
  //RTC_DCHECK(thread_);
    //zhangpeng 20170419
    //目前thread_是主线程，主线程不是webrtc创建的，主线程不会拉取post的消息
    //如果thread_是一个webrtc创建的Thread对象里启动的工作线程，它才会在它的run函数中循环拉取消息
    //thread_->Post(RTC_FROM_HERE, this, kMessageTypeInterruptionBegin);
    
    //故意做了延时调用，怀疑电话挂断后，还没有来得时释放资源时，这里就reset可能会无效
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if (g_audioDeviceIsValid) {
