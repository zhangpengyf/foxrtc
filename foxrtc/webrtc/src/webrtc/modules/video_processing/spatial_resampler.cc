...
int32_t VPMSimpleSpatialResampler::ResampleFrame(const VideoFrame& inFrame,
    ...
    //下面两行的目的是：
    //外部设置视频宽高时不需要考虑宽和高哪个值大，以及手机的角度
    //这里会检测到设置的宽高和图像的宽高刚好相反，就会进行调整
    if (
        ((inFrame.width()>inFrame.height())^(target_width_>target_height_))) {
        int32_t temp = target_height_;
        target_height_ = target_width_;
        target_width_ = temp;
    }
