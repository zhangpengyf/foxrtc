...
AudioProcessingImpl::AudioProcessingImpl(const webrtc::Config& config,
	...
	before_level_control(NULL),
	after_level_control(NULL),
#endif
                         ...
                         config.Get<Intelligibility>().enabled) {
  {
	...
	if (before_aec == NULL)
	{
		before_aec = fopen("e://before_aec.pcm", "wb");
	}
	if (before_ns == NULL)
	{
		before_ns = fopen("e://before_ns.pcm", "wb");
	}
	if (before_aecm == NULL)
	{
		before_aecm = fopen("e://before_aecm.pcm", "wb");
	}
	if (before_gain_control == NULL)
	{
		before_gain_control = fopen("e://before_gain_control.pcm", "wb");
	}
	if (before_transient == NULL)
	{
		before_transient = fopen("e://before_transient.pcm", "wb");
	}
	if (before_level_control == NULL)
	{
		before_level_control = fopen("e://before_level_control.pcm", "wb");
	}
	if (after_level_control == NULL)
	{
		after_level_control = fopen("e://after_level_control.pcm", "wb");
	}
#endif
...
AudioProcessingImpl::~AudioProcessingImpl() {
  ...
  if (before_level_control != NULL)
  {
	  fclose(before_level_control);
	  before_level_control = NULL;
  }
  if (after_level_control != NULL)
  {
	  fclose(after_level_control);
	  after_level_control = NULL;
  }
#endif
...
int AudioProcessingImpl::ProcessCaptureStreamLocked() {
  ...
  if (before_aec != NULL)
  {
	  ...
	  {
		  fwrite(capture_buffer->channels()[0], 2, capture_buffer->num_frames_per_band(), before_aec);
	  }
  }
#endif
  ...
  if (before_ns != NULL)
  {
	  for (i = 0; i < capture_buffer->num_channels(); i++)
	  {
		  fwrite(capture_buffer->channels()[0], 2, capture_buffer->num_frames_per_band(), before_ns);
	  }
  }
#endif
  ...
  if (before_aecm != NULL)
  {
	  for (i = 0; i < capture_buffer->num_channels(); i++)
	  {
		  fwrite(capture_buffer->channels()[0], 2, capture_buffer->num_frames_per_band(), before_aecm);
	  }
  }
#endif
  ...
  if (before_gain_control != NULL)
  {
	  for (i = 0; i < capture_buffer->num_channels(); i++)
	  {
		  fwrite(capture_buffer->channels()[0], 2, capture_buffer->num_frames_per_band(), before_gain_control);
	  }
  }
#endif
  ...
  if (before_transient != NULL)
  {
	  for (i = 0; i < capture_buffer->num_channels(); i++)
	  {
		  fwrite(capture_buffer->channels()[0], 2, capture_buffer->num_frames_per_band(), before_transient);
	  }
  }
#endif
  if (capture_.transient_suppressor_enabled) {
  ...
  if (before_level_control != NULL)
  {
	  for (i = 0; i < capture_buffer->num_channels(); i++)
	  {
		  fwrite(capture_buffer->channels()[0], 2, capture_buffer->num_frames_per_band(), before_level_control);
	  }
  }
#endif
  ...
  if (after_level_control != NULL)
  {
	  for (i = 0; i < capture_buffer->num_channels(); i++)
	  {
		  fwrite(capture_buffer->channels()[0], 2, capture_buffer->num_frames_per_band(), after_level_control);
	  }
  }
#endif
