  ...
  if (ExplicitlyConfiguredSpatialLayers()) {
    ...
    for (i = 0; i < num_spatial_layers_; ++i) {
      ...
      if (num_temporal_layers_ == 1) {
        ...
        config_->layer_target_bitrate[i * num_temporal_layers_] =
            (config_->ss_target_bitrate[i] * 11) / 32;
        config_->layer_target_bitrate[i * num_temporal_layers_ + 1] =
            config_->layer_target_bitrate[i * num_temporal_layers_] +
            ((config_->ss_target_bitrate[i] * 5) / 16);
        config_->layer_target_bitrate[i * num_temporal_layers_ + 2] =
            config_->ss_target_bitrate[i];
        LOG(LS_INFO) << "layer_target_bitrate 0 = " << config_->layer_target_bitrate[0];
        LOG(LS_INFO) << "layer_target_bitrate 1 = " << config_->layer_target_bitrate[1];
        LOG(LS_INFO) << "layer_target_bitrate 2 = " << config_->layer_target_bitrate[2];
      } else {
...
int VP9EncoderImpl::InitEncode(const VideoCodec* inst,
  ...
  LOG(LS_INFO) << "num_temporal_layers_ = " << (int)inst->codecSpecific.VP9.numberOfTemporalLayers
               << "keyFrameInterval = " << inst->codecSpecific.VP9.keyFrameInterval
               << ", num_spatial_layers_ = " << (int)num_spatial_layers_;
  ...
  if (is_flexible_mode_) {
    ...
    layer_flags_[0] = VPX_EFLAG_FORCE_KF | VP8_EFLAG_NO_REF_GF |
                     VP8_EFLAG_NO_REF_ARF | VP8_EFLAG_NO_UPD_GF |
                     VP8_EFLAG_NO_UPD_ARF;
    layer_flags_[1] = VP8_EFLAG_NO_REF_GF | VP8_EFLAG_NO_REF_ARF |
                     VP8_EFLAG_NO_UPD_ARF | VP8_EFLAG_NO_UPD_LAST;
  } else if (num_temporal_layers_ == 3) {
    LOG(LS_INFO) << "mode 0212";
    ...
    layer_flags_[0] = VP8_EFLAG_NO_REF_GF |
                     VP8_EFLAG_NO_REF_ARF | VP8_EFLAG_NO_UPD_GF |
                     VP8_EFLAG_NO_UPD_ARF;
    layer_flags_[2] = VP8_EFLAG_NO_REF_GF | VP8_EFLAG_NO_REF_ARF |
                     VP8_EFLAG_NO_UPD_ARF | VP8_EFLAG_NO_UPD_LAST;
    layer_flags_[1] = layer_flags_[3] =
...
int VP9EncoderImpl::InitAndSetControlSettings(const VideoCodec* inst) {
  ...
  LOG(LS_INFO) << "cpu_speed_ = " << cpu_speed_;
  LOG(LS_INFO) << "rc_max_intra_target_ = " << rc_max_intra_target_;
  LOG(LS_INFO) << "adaptiveQpMode = " << inst->codecSpecific.VP9.adaptiveQpMode;
  ...
  if (num_temporal_layers_ > 1 || num_spatial_layers_ > 1) {
    LOG(LS_INFO) << "svc_internal_.svc_params.max_quantizers[0] = "
                 << svc_internal_.svc_params.max_quantizers[0];
    LOG(LS_INFO) << "svc_internal_.svc_params.min_quantizers[0] = "
                 << svc_internal_.svc_params.min_quantizers[0];

    LOG(LS_INFO) << "svc_internal_.svc_params.scaling_factor_num[0] = "
                 << svc_internal_.svc_params.scaling_factor_num[0];
    LOG(LS_INFO) << "svc_internal_.svc_params.scaling_factor_den[0] = "
                 << svc_internal_.svc_params.scaling_factor_den[0];
  ...
  LOG(LS_INFO) << "config_->g_threads = " << config_->g_threads;
  ...
  if(inst->codecSpecific.VP9.denoisingOn) {
    LOG(LS_INFO) << "denoisingOn = true";
  }
  else {
    LOG(LS_INFO) << "denoisingOn = false";
  }
  ...
  if (codec_.mode == kScreensharing) {
    ...
    vpx_codec_control(encoder_, VP9E_SET_TUNE_CONTENT, 0);
  }
...
int VP9EncoderImpl::Encode(const VideoFrame& input_image,
  ...
  if (send_keyframe) {
    ...
    LOG(LS_INFO) << " Keyframe ";

  }
  else {
    vpx_svc_layer_id layer_id;
    layer_id.temporal_layer_id =
        config_->ts_layer_id[frame_cnt_ % config_->ts_periodicity];
    layer_id.spatial_layer_id = 0;
    vpx_codec_control(encoder_, VP9E_SET_SVC_LAYER_ID, &layer_id);
    flags = layer_flags_[frame_cnt_ % config_->ts_periodicity];
  }
