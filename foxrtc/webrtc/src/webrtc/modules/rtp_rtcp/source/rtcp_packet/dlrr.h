...
struct ReceiveTimeInfo {
  // RFC 3611 4.5
  ReceiveTimeInfo() : ssrc(0), last_rr(0), delay_since_last_rr(0),add_delay(0) {}
  ReceiveTimeInfo(uint32_t ssrc, uint32_t last_rr, uint32_t delay)
      : ssrc(ssrc), last_rr(last_rr), delay_since_last_rr(delay),add_delay(0) {}
  uint32_t ssrc;
  uint32_t last_rr;
...
class Dlrr {
 ...
 private:
  ...
  static const size_t kSubBlockLength = 16;
