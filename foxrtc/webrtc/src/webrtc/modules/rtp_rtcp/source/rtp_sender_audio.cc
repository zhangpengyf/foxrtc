...
bool RTPSenderAudio::SendAudio(FrameType frame_type,
  ...
  // Update audio level extension, if included.
    ...
    if (fragmentation && fragmentation->fragmentationVectorSize > 0) {
        int len = 0;
        for(int i = fragmentation->fragmentationVectorSize-1;i>=0;i--){
            int redHeader = 4;
            if(i == 0)redHeader = 1;
            len +=redHeader;
            len +=fragmentation->fragmentationLength[i];
        }

        // Use the fragment info if we have one.
        uint8_t* payload =
        packet->AllocatePayload(len);
        ...
        for(int i = fragmentation->fragmentationVectorSize-1;i>=0;i--){
            if(i==0){
                payload[index] = fragmentation->fragmentationPlType[i]&0x7f;
                index++;
            }
            else{
                payload[index] = fragmentation->fragmentationPlType[i]|0x80;
                payload[index+1] = (fragmentation->fragmentationTimeDiff[i]>>6)&0xff;
                payload[index+2] = (fragmentation->fragmentationTimeDiff[i]<<2)&0xfc;
                payload[index+2] = payload[index+2] + ((fragmentation->fragmentationLength[i]>>8)&0x03);
                payload[index+3] = fragmentation->fragmentationLength[i]&0xff;
                index+=4;
            }
        }
        for(int i = fragmentation->fragmentationVectorSize-1;i>=0;i--){
            memcpy(payload+index, payload_data+fragmentation->fragmentationOffset[i], fragmentation->fragmentationLength[i]);
            index+=fragmentation->fragmentationLength[i];
        }
    } else {
